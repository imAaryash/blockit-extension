...
// Check daily login and show reward modal
async function checkDailyLogin() {
  try {
    ...
    if (!token) {
      console.error('[Daily Login] No auth token found');
      return;
    }
    
    const response = await fetch(`${API_URL}/season/daily-login`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
    ...
    if (!response.ok) {
      console.error('[Daily Login] Failed to check');
      return;
    }
    ...
    if (data.success && data.isNewDay) {
      console.log('[Daily Login] New day detected! Showing modal...');
      // Show daily login modal
      showDailyLoginModal(data);
    } else {
      console.log('[Daily Login] Not a new day or already claimed today');
    }
...
function showDailyLoginModal(data) {
  const { currentStreak, reward, newBalance, streakBroken } = data;
  
  console.log('[Daily Login Modal] Showing modal with data:', data);
  
  // Define reward tiers - professional milestone-based
  const rewardTiers = [
    { days: '1-2', reward: 10, label: 'Starter' },
    { days: '3-6', reward: 25, label: 'Consistent' },
    { days: '7-13', reward: 50, label: 'Committed' },
    { days: '14-29', reward: 100, label: 'Dedicated' },
    { days: '30+', reward: 200, label: 'Master' }
  ];
  
  // Create modal if it doesn't exist
  let modal = document.querySelector('.daily-login-modal');
  if (!modal) {
    modal = document.createElement('div');
    modal.className = 'daily-login-modal';
    modal.innerHTML = `
      <div class="daily-login-content">
        <div class="daily-login-ribbon">
          <h2>Daily Login Bonus</h2>
        </div>
        
        <div class="daily-login-subtitle">
          Consistent daily logins earn you bonus coins
        </div>
        
        <div class="daily-reward-grid">
          ${rewardTiers.map((tier, index) => {
            const isActive = getDayTier(currentStreak) === index;
            const isPast = getDayTier(currentStreak) > index;
            const status = isPast ? 'claimed' : (isActive ? 'active' : 'locked');
            
            return `
              <div class="reward-day ${status}">
                <div class="day-label">${tier.label}</div>
                <div class="day-sublabel">Day ${tier.days}</div>
                <div class="day-reward">+${tier.reward} coins</div>
                ${isPast ? '<div class="checkmark">✓</div>' : ''}
                ${status === 'locked' ? '<div class="lock-icon">🔒</div>' : ''}
              </div>
            `;
          }).join('')}
          
          <div class="reward-day special ${currentStreak >= 7 ? 'active' : 'locked'}">
            <div class="day-label">Week Milestone</div>
            <div class="day-sublabel">Day 7+</div>
            <div class="day-reward">Bonus Unlocked</div>
          </div>
        </div>
        
        ${streakBroken ? '<div class="streak-broken-notice">⚠️ Your streak was reset. Start building a new one today.</div>' : ''}
        
        <div class="daily-login-stats">
          <div class="stat-item">
            <div class="stat-label">Current Streak</div>
            <div class="stat-value">${currentStreak} ${currentStreak === 1 ? 'day' : 'days'}</div>
          </div>
          <div class="stat-item">
            <div class="stat-label">Today's Reward</div>
            <div class="stat-value earned">+${reward}</div>
          </div>
          <div class="stat-item">
            <div class="stat-label">Total Balance</div>
            <div class="stat-value">${newBalance}</div>
          </div>
        </div>
        
        <button class="close-login-modal">Claim Reward</button>
      </div>
    `;
    ...
    modal.querySelector('.close-login-modal').addEventListener('click', () => {
      ...
      if (seasonData) {
        seasonData.season.shopCoins = newBalance;
        const coinAmount = document.querySelector('.coin-amount');
        if (coinAmount) {
          coinAmount.textContent = newBalance;
          coinAmount.style.animation = 'none';
          setTimeout(() => {
            coinAmount.style.animation = 'pulse 0.5s ease';
          }, 10);
        }
      }
    });
...
// Helper function to determine which tier a day falls into
